...
$id=$_GET["id"];
$sql=$conexion->query("SELECT carga.id_carga As idcarga, carga.fecha_elab As fecha_elab, carga.numero_carga As ncarga, cliente.nombre_cliente As cliente, carga.cantidad_sacos_asignados As cantsacos, carga.fase As fase, carga.estado As estado FROM carga,cliente WHERE (carga.id_carga = $id) AND (carga.id_cliente = cliente.id_cliente)");
?>
<!DOCTYPE html>
<html lang="es">
    <head>
    ...
    <aside
            ...
            <div>
                ...
                <div class="mt-8 text-center">
                    ...
                    <span class="hidden text-gray-400 lg:block cursor-defaul select-nonet">Jefe de Área</span>
                </div>
    ...
    <div class="ml-auto lg:w-[75%] xl:w-[80%] 2xl:w-[85%]">
        ...
        <div class="flex items-center justify-between font-mont font-extrabold text-indigo-900 text-4xl ml-4">
            <h1 class="cursor-default select-none">ELIMINAR CARGA</h1>
            <a href="jefe_area_maxisacos_1.php" class="bg-indigo-800 hover:bg-indigo-900 text-white select-none font-mont font-bold px-10 py-2 text-lg rounded-xl mr-9">VOLVER</a>
        </div>
        ...
        <?php
            while ($datos = $sql->fetch_object()) {
                $fecha_elaboracion = date("d-m-Y", strtotime($datos->fecha_elab));
            ?>
            <p class="bg-indigo-800 rounded-2xl py-2 cursor-default select-none">N° Carga</p>
            <input class="w-72 rounded-xl text-black pl-4 font-medium outline-none select-none" type="text" name="txtnCarga" readonly value="<?= $datos-> ncarga ?>">

            <p class="bg-indigo-800 rounded-2xl py-2 cursor-default select-none">Nombre Cliente</p>
            <input class="w-72 rounded-xl text-black pl-4 font-medium outline-none select-none" type="text" name="nomCliente" readonly value="<?= $datos-> cliente ?>">

            <p class="bg-indigo-800 rounded-2xl py-2 cursor-default select-none">Cantidad de Sacos</p>
            <input class="w-72 rounded-xl text-black pl-4 font-medium outline-none select-none" type="text" name="txtcantSacos" readonly value="<?= $datos-> cantsacos ?>">

            <p class="bg-indigo-800 rounded-2xl py-2 cursor-default select-none">Estado</p>
            <input class="w-72 rounded-xl text-black pl-4 font-medium outline-none select-none" type="text" name="txtestado" readonly value="<?= $datos-> estado ?>">

            <p class="bg-indigo-800 rounded-2xl py-2 cursor-default select-none">Fase</p>
            <input class="w-72 rounded-xl text-black pl-4 font-medium outline-none select-none" type="text" name="txtfase" readonly value="<?= $datos-> fase ?>">
            <p class="bg-indigo-800 rounded-2xl py-2 cursor-default select-none">Fecha Elaboración</p>
            <input class="w-72 rounded-xl text-black pl-4 font-medium outline-none select-none" type="text" name="txtfecha" readonly value="<?= $fecha_elaboracion ?>">
        <?php }?>
        </div>
        ...
        <?php
            ...
            $sql = "UPDATE carga SET fase='$fase', estado='$estado' WHERE (id_carga='$id')";
            mysqli_query($conexion, $sql);
            echo "<script>Swal.fire({
                title: 'Exito',
                text: 'Se ha eliminado la carga exitosamente',
                icon: 'success'
              });</script>";
            }             
        ?>        
