...
$rs = mysqli_query($conexion, "SELECT operador.id_operador, operador.nombre_operador, operador.apellido_operador, turno.id_turno, turno.turno, login.usuario, login.contraseña, login.id_login FROM operador, turno, login WHERE (operador.id_operador = '$id') AND (operador.turno = turno.id_turno) AND (operador.id_login = login.id_login)");
...
<nav class="ml-auto mb-6 lg:w-[75%] xl:w-[80%] 2xl:w-[85%]">
    ...
    <div class="ml-auto lg:w-[75%] xl:w-[80%] 2xl:w-[85%]">
        ...
        <div class="flex items-center justify-between font-mont font-extrabold text-indigo-900 text-4xl ml-4">
            <h1 class="cursor-default select-none">EDITAR OPERADOR</h1>
        ...
        <div class="grid grid-cols-2 gap-x-4 gap-y-10 pt-12 pb-8 ml-20 w-96 text-center text-white font-mont font-bold">
            ...
            <input class="w-72 rounded-xl text-black pl-4 font-medium select-none" oninput="this.value = this.value.replace(/[^A-Za-z]/g, '')"  type="text" placeholder="Inserte Nombre" name="txtNombre" required value="<?php echo $nombre_operador ?>">
            <p class="bg-indigo-800 rounded-2xl py-2 cursor-default select-none">Apellido Operador</p>
            <input class="w-72 rounded-xl text-black pl-4 font-medium select-none" oninput="this.value = this.value.replace(/[^A-Za-z]/g, '')"  type="text" placeholder="Inserte Apellido" name="txtApe" required value="<?php echo $apellido_operador ?>">
            
            <p class="bg-indigo-800 rounded-2xl py-2 cursor-default select-none">Turno Operador</p>
            <?php
                $sql1 = "SELECT id_turno, turno FROM turno";
                $result = mysqli_query($conexion, $sql1);
            ?>
            <select class="w-72 rounded-xl pl-2 text-black font-medium cursor-pointer outline-none select-none" name="cboturno">
                <option value='<?php echo $id_turno ; ?>'><?php echo $turno; ?></option>
                <option value='<?php echo $id_turno ; ?>'>--------------------</option>
                <?php
            ...
            <input class="w-72 rounded-xl text-black pl-4 font-medium select-none"  type="text" placeholder="Inserte Usuario" name="txtUsu" required value="<?php echo $usuario  ?>">
            <p class="bg-indigo-800 rounded-2xl py-2 cursor-default select-none">Contraseña</p>
            <input class="w-72 rounded-xl text-black pl-4 font-medium select-none"  type="text" placeholder="Inserte Contraseña" name="txtPass" required value="<?php echo $contraseña  ?>">
        ...
        <?php
            if (isset($_POST['btnActualizar']) && $_POST['btnActualizar'] == "Actualizar") {
                ...
                $Turno = ($_POST['cboturno'] != '') ? $_POST['cboturno'] : $id_turno;
                $Usuario = $_POST['txtUsu'];
                $Contraseña = $_POST['txtPass'];
            
                $sql = "UPDATE login,operador SET login.usuario='$Usuario', login.contraseña='$Contraseña', operador.nombre_operador='$Nombre', operador.apellido_operador='$Apellido', operador.turno='$Turno' WHERE (operador.id_operador= '$id') AND (operador.id_login = login.id_login)";
