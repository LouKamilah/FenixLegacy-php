...
$rs = mysqli_query($conexion, "SELECT jefe_area.id_jefeArea, jefe_area.nombre_jefeArea, jefe_area.apellido_jefeArea, login.usuario, login.contraseña, login.id_login FROM jefe_area, login WHERE (jefe_area.id_jefeArea = '$id') AND (jefe_area.id_login = login.id_login)");
if ($row = mysqli_fetch_array($rs)) {
    $id_jefeArea = $row[0];
    $nombre_jefeArea = $row[1];
    $apellido_jefeArea = $row[2];
    $usuario = $row[3];
    $contraseña = $row[4];
    $IDLogin = $row[5];
}
...
<div class="ml-auto lg:w-[75%] xl:w-[80%] 2xl:w-[85%]">
    ...
    <div class="flex items-center justify-between font-mont font-extrabold text-indigo-900 text-4xl ml-4">
        <h1 class="cursor-default select-none">EDITAR JEFE AREA</h1>
        ...
        <div class="grid grid-cols-2 gap-x-4 gap-y-10 pt-12 pb-8 ml-20 w-96 text-center text-white font-mont font-bold">
            ...
            <input class="w-72 rounded-xl text-black pl-4 font-medium select-none" oninput="this.value = this.value.replace(/[^A-Za-z]/g, '')"  type="text" placeholder="Inserte Nombre" name="txtNombre" required value="<?php echo $nombre_jefeArea   ?>">
            <p class="bg-indigo-800 rounded-2xl py-2 cursor-default select-none">Apellido Jefe Area</p>
            <input class="w-72 rounded-xl text-black pl-4 font-medium select-none" oninput="this.value = this.value.replace(/[^A-Za-z]/g, '')"  type="text" placeholder="Inserte Apellido" name="txtApe" required value="<?php echo $apellido_jefeArea  ?>">
        ...
        <?php
            if (isset($_POST['btnActualizar']) && $_POST['btnActualizar'] == "Actualizar") {
                ...
                $sql = "UPDATE login,jefe_area SET login.usuario='$Usuario', login.contraseña='$Contraseña', jefe_area.nombre_jefeArea='$Nombre', jefe_area.apellido_jefeArea='$Apellido' WHERE (jefe_area.id_jefeArea= '$id') AND (jefe_area.id_login = login.id_login)";
