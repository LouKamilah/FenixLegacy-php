...
$id=$_GET["id"];
$sql=$conexion->query("SELECT operador.id_operador, operador.nombre_operador, operador.apellido_operador, turno.id_turno, turno.turno, login.usuario, login.contraseña, login.id_login FROM operador, turno, login WHERE (operador.id_operador = '$id') AND (operador.turno = turno.id_turno) AND (operador.id_login = login.id_login)");
?>
<!DOCTYPE html>
<html lang="es">
    <head>
...
<nav class="ml-auto mb-6 lg:w-[75%] xl:w-[80%] 2xl:w-[85%]">
    ...
    <div class="ml-auto lg:w-[75%] xl:w-[80%] 2xl:w-[85%]">
        ...
        <div class="flex items-center justify-between font-mont font-extrabold text-indigo-900 text-4xl ml-4">
            <h1 class="cursor-default select-none">ELIMINAR OPERADOR</h1>
        ...
        <?php
            ...
            ?>
            <p class="bg-indigo-800 rounded-2xl py-2 cursor-default select-none">ID Operador</p>
            <input class="w-72 rounded-xl text-black pl-4 font-medium outline-none select-none" type="text" name="txtID" readonly value="<?= $datos-> id_operador ?>">

            <p class="bg-indigo-800 rounded-2xl py-2 cursor-default select-none">Nombre Operador</p>
            <input class="w-72 rounded-xl text-black pl-4 font-medium outline-none select-none" type="text" name="txtNom" readonly value="<?= $datos-> nombre_operador ?>">

            <p class="bg-indigo-800 rounded-2xl py-2 cursor-default select-none">Apellido Operador</p>
            <input class="w-72 rounded-xl text-black pl-4 font-medium outline-none select-none" type="text" name="txtApe" readonly value="<?= $datos-> apellido_operador ?>">

            <p class="bg-indigo-800 rounded-2xl py-2 cursor-default select-none">Turno</p>
            <input class="w-72 rounded-xl text-black pl-4 font-medium outline-none select-none" type="text" name="txtTurno" readonly value="<?= $datos-> turno ?>">
        ...
        <?php
            ...
            $insertarUno = $conexion->query("DELETE FROM operador where id_operador='$IDOp'");
            if ($insertarUno==true) {
