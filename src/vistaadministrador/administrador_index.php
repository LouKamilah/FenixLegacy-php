...
<body>
    ...
    <div class="ml-auto lg:w-[75%] xl:w-[80%] 2xl:w-[85%]">
        ...
        <div class="font-mont font-extrabold text-indigo-900 text-4xl ml-4">
            <h1 class="cursor-default select-none">REGISTRO OPERADORES</h1>
        </div>
        ...
        <div class="flex flex-row justify-between mt-5">
            <div
                ...
                <a href="crear_op.php">Nuevo Operador</a>
            </div>
        </div>
            ...
            <div>
                <table class="mt-8 min-w-full border-collapse block md:table" id="tblDatos">
                    ...
                    <tbody class="block md:table-row-group">
                        <?php
                        $sql = $conexion->query("SELECT operador.id_operador, operador.nombre_operador, operador.apellido_operador, turno.turno, operador.id_login, login.usuario, login.contraseña
                        FROM operador,turno,login
                        WHERE operador.turno = turno.id_turno AND operador.id_login = login.id_login ORDER BY operador.id_operador ASC");
                        while ($datos = $sql->fetch_object()) {
                         ?>
                            <tr class="h-14 bg-gray-300 border border-grey-500 md:border-none block md:table-row text-center">
                                <td><?= $datos->id_operador ?></td>
                                <td><?= $datos->nombre_operador ?></td>
                                <td><?= $datos->apellido_operador ?></td>
                                <td><?= $datos->turno ?></td>
                                ...
                                <td>
                                    <div class="flex flex-row space-x-4 justify-center">
                                        ...
                                        <a href="editar_op.php?id=<?= $datos->id_operador ?>">Editar</a>
                                        <button class="bg-red-500 hover:bg-red-700 text-white font-bold py-1 px-2 border w-20 border-red-500 rounded">
                                        <a href="eliminar_op.php?id=<?= $datos->id_operador ?>">Eliminar</a></button>
                                    </div>
                                </td>
                                </td>
                            </tr>
