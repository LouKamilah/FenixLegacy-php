...
<body>
	...
	<div class="ml-auto lg:w-[75%] xl:w-[80%] 2xl:w-[85%]">
		<div class="font-mont font-extrabold text-indigo-900 text-4xl ml-4">
			<h1 class="cursor-default">LISTA COMPLETA</h1>
		</div>
		<div>
			...
			<form method="POST" class="flex flex-col justify-center mt-4 py-6 px-12 rounded-xl ">
				...
				<table class=" min-w-full border-collapse md:table" id="tblDatos">
					...
					if (isset($_POST["buscar"]) && !empty($_POST["buscar"])) {
						$filtro = $_POST["buscar"];
						$sql = "SELECT c.numero_carga, c.fecha_elab, c.id_carga, cl.nombre_cliente, c.cantidad_sacos_asignados FROM carga c INNER JOIN cliente cl ON c.id_cliente = cl.id_cliente WHERE cl.nombre_cliente LIKE '%$filtro%'";
					} else {
						$sql = "SELECT c.numero_carga, c.fecha_elab, c.id_carga, cl.nombre_cliente, c.cantidad_sacos_asignados FROM carga c INNER JOIN cliente cl ON c.id_cliente = cl.id_cliente";
					}
					...
					while ($mostrar = mysqli_fetch_array($result)) {
						...
						<tbody class="block md:table-row-group">
							<tr
								...
								<td><?php echo $mostrar['id_carga'] ?></td>
								<td><?php echo $mostrar['numero_carga'] ?></td>
								<td><?php echo $mostrar['nombre_cliente'] ?></td>
								<td><?php echo $mostrar['cantidad_sacos_asignados'] ?></td>
                                <td><?php echo date("d-m-Y", strtotime($mostrar['fecha_elab'])) ?></td>
								<td>
									<a href="lista_sacos.php?id_carga=<?php echo $mostrar['id_carga']; ?>" class=" bg-indigo-800 hover:bg-indigo-900 text-white font-bold py-1 w-32 border px-7 border-indigo-900 rounded">Ver Sacos</a>
								</td>
							</tr>
							<?php
					}
