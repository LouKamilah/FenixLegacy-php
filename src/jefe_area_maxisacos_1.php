...
<body>
    ...
    if (!isset($_POST['buscarEstado'])) {
        $_POST['buscarEstado'] = '';
    }
    if (!isset($_POST['buscarFase'])) {
        $_POST['buscarFase'] = '';
    }
    ?>
    ...
    <div class="ml-auto lg:w-[75%] xl:w-[80%] 2xl:w-[85%]">
        ...
        <div class="font-mont font-extrabold text-indigo-900 text-4xl ml-4">
            <h1 class="cursor-default select-none">LISTA MAXISACOS</h1>
        </div>

        <div class="flex flex-row justify-between mt-5">
            <div
                class="flex items-center justify-center h-10 w-80 ml-48 cursor-pointer select-none bg-indigo-800 rounded-2xl font-semibold text-xl text-blue-50 hover:bg-indigo-900">
                <a href="crear_carga.php">Crear Carga</a>
            </div>


            <div
                class="flex items-center justify-center w-80 mr-56 cursor-pointer select-none bg-indigo-800 rounded-2xl font-semibold text-xl text-blue-50 hover:bg-indigo-900">
        ...
        <div class="flex items-center justify-end mr-48 px-4">
            <table class="table -mr-16">
                <thead>
                    <tr class="filtros">
                    <input class="px-16 py-2 border-2 border-indigo-900 rounded-xl focus:outline-none" type="text" id="buscar" name="buscar" value="<?php echo $_POST["buscar"] ?>" placeholder="Buscar Cliente">
                        <td>
                            <p class="text-center font-mont font-semibold cursor-default bg-indigo-800 text-white rounded-xl ml-6 w-18">Estado</p>
                                <select class="text-center font-mont mr-4 rounded-full focus:outline-none font-medium " name="buscarEstado" id="buscarEstado">
                                    <?php if ($_POST["buscarEstado"] != '') { ?>
                                        <option value="<?php echo $_POST["buscarEstado"]; ?>">
                                            <?php echo $_POST["buscarEstado"]; ?>
                                        </option>
                                    <?php } ?>
                            ...
                            <td>
                                <p class="text-center font-mont font-semibold cursor-default bg-indigo-800 text-white rounded-xl ml-14 w-18">Fase</p>
                                <select class="text-center font-mont ml-4 rounded-full focus:outline-none font-medium " name="buscarFase" id="buscarFase">
                                    <?php if ($_POST["buscarFase"] != '') { ?>
                                        <option value="<?php echo $_POST["buscarFase"]; ?>">
                                            <?php echo $_POST["buscarFase"]; ?>
                                        </option>
                                    <?php } ?>
                                    <option value="Todos">Todos</option>
                            ...
                            <th>
                                <input class="bg-indigo-800 hover:bg-indigo-900 cursor-pointer text-white font-bold py-2 w-36 rounded-2xl ml-9" type="submit" value="Buscar" name="Buscar">
                            </th>
                        </tr>
                    </thead>
                </table>
                ...
                if ($_POST['Buscar'] == "Buscar") {
                    if ($_POST["buscar"] == '' and $_POST["buscarEstado"] == 'Todos' and $_POST["buscarFase"] == 'Todos') {
                        $filtro = "";
                    } else {
                        if ($_POST["buscar"] != '' and $_POST["buscarEstado"] == 'Todos' and $_POST["buscarFase"] == 'Todos') {
                            $filtro = "AND cliente.nombre_cliente = '" . $_POST["buscar"] . "'";
                        }

                        if ($_POST["buscar"] != '' and $_POST["buscarEstado"] != 'Todos' and $_POST["buscarFase"] == 'Todos') {
                            $filtro = "AND cliente.nombre_cliente = '" . $_POST["buscar"] . "' AND estado = '" . $_POST["buscarEstado"] . "' ";
                        }

                        if ($_POST["buscar"] != '' and $_POST["buscarEstado"] != 'Todos' and $_POST["buscarFase"] != 'Todos') {
                            $filtro = "AND cliente.nombre_cliente = '" . $_POST["buscar"] . "' AND estado = '" . $_POST["buscarEstado"] . "' AND fase = '" . $_POST["buscarFase"] . "'";
                        }

                        if ($_POST["buscar"] == '' and $_POST["buscarEstado"] != 'Todos' and $_POST["buscarFase"] == 'Todos') {
                            $filtro = "AND estado = '" . $_POST["buscarEstado"] . "'";
                        }

                        if ($_POST["buscar"] == '' and $_POST["buscarEstado"] != 'Todos' and $_POST["buscarFase"] != 'Todos') {
                            $filtro = "AND estado = '" . $_POST["buscarEstado"] . "' AND fase = '" . $_POST["buscarFase"] . "'";
                        }

                        if ($_POST["buscar"] == '' and $_POST["buscarEstado"] == 'Todos' and $_POST["buscarFase"] != 'Todos') {
                            $filtro = "AND fase = '" . $_POST["buscarFase"] . "'";
                        }

                        if ($_POST["buscar"] != '' and $_POST["buscarEstado"] == 'Todos' and $_POST["buscarFase"] != 'Todos') {
                            $filtro = "AND cliente.nombre_cliente = '" . $_POST["buscar"] . "' AND fase = '" . $_POST["buscarFase"] . "'";
                        }
                    }
                ...
                <table class="mt-8 min-w-full border-collapse block md:table" id="tblDatos">
                    <thead class="block md:table-header-group">
                        <tr class="bg-indigo-800 text-white font-bold h-10">
                            ...
                            <th class="">Estado</th>
                            <th class="">Fase</th>
                            <th class="">Fecha Elaboración</th>
                            <th></th>
                        </tr>
                    </thead>
                    <tbody class="block md:table-row-group">
                        <?php
                        $sql = $conexion->query("SELECT carga.id_carga, carga.fecha_elab, carga.numero_carga, cliente.nombre_cliente As nomCliente, carga.cantidad_sacos_asignados, carga.fase, carga.estado FROM carga, cliente WHERE (carga.id_cliente = cliente.id_cliente) $filtro");
                        while ($datos = $sql->fetch_object()) {
                            $fecha_mostrar = date("d-m-Y", strtotime($datos->fecha_elab)); ?>
                            <tr class="h-14 bg-gray-300 border border-grey-500 md:border-none block md:table-row text-center">
                                <td><?= $datos->numero_carga ?></td>
                                <td><?= $datos->nomCliente ?></td>
                                <td><?= $datos->cantidad_sacos_asignados ?></td>
                                <td><?= $datos->estado ?></td>
                                <td><?= $datos->fase ?></td>
                                <td><?= $fecha_mostrar ?></td>
                                <td>
                                    <div class="flex flex-row space-x-4 justify-center">
                                        ...
                                        <a href="editar_carga.php?id=<?= $datos->id_carga ?>">Editar</a>
                                        <button class="bg-red-500 hover:bg-red-700 text-white font-bold py-1 px-2 border w-20 border-red-500 rounded">
                                        <a href="eliminar_carga.php?id=<?= $datos->id_carga ?>">Eliminar</a></button>
                                    </div>
                                </td>
                                </td>
                            </tr>
